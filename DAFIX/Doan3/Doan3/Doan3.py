...
def predict_score(title):
    ...
    try:
        ...
        for neighbor in neighbors:
            avgMoney = avgMoney + motels.iloc[neighbor[0]][9]
            print(neighbor)
    
        print('\n')

        avgMoney = avgMoney/K
        print('The predicted money for %s là: %f' %(new_motel['Tiêu đề tin'].values[0],avgMoney))
        print('The actual money for %s là %f' %(new_motel['Tiêu đề tin'].values[0],new_motel['Giá']))
        mse = float(new_motel['Giá'].values[0])/avgMoney
        rmse = np.sqrt(mse)
...
for numberrow in range(0, row_count , 1):
    ...
    predict_score(moteltest['Tiêu đề tin'][numberrow]);
    print("/n")
